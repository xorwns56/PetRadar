...
const RegisterForm = ({ isExist, onCreate, onRegister }) => {
  const formCheck = (name) => {
    ...
    if (!input.pw) {
      ...
      newErrMsg.pw = '비밀번호는 영문, 숫자, 특수문자를 포함한 8자 이상이어야 합니다.';
    }
  ...
  return (
    <div className="RegisterForm">
      <form onSubmit={onSubmit} autoComplete="off">
        <div className={`input_item id ${focus.id ? 'focus' : ''} ${input.id ? 'on' : ''} ${errMsg.id ? 'err' : ''}`}>
          <input
            type="text"
            name="id"
            id="register_id"
        ...
        <div className={`input_item pw ${focus.pw ? 'focus' : ''} ${input.pw ? 'on' : ''} ${errMsg.pw ? 'err' : ''}`}>
          <input
        ...
        <div className={`input_item hp ${focus.hp ? 'focus' : ''} ${input.hp ? 'on' : ''} ${errMsg.hp ? 'err' : ''}`}>
          <input
            type="text"
            name="hp"
            id="register_hp"
        ...
        <div className={`error_message ${errMsg.id ? '' : 'hide'}`}>{errMsg.id}</div>
        <div className={`error_message ${errMsg.pw ? '' : 'hide'}`}>{errMsg.pw}</div>
        <div className={`error_message ${errMsg.hp ? '' : 'hide'}`}>{errMsg.hp}</div>
        <button
          type="submit"
          className={`btn_login ${
            !input.id || !input.pw || !input.hp || errMsg.id || errMsg.pw || errMsg.hp ? '' : 'on'
          }`}
        >
          회원가입
        </button>
